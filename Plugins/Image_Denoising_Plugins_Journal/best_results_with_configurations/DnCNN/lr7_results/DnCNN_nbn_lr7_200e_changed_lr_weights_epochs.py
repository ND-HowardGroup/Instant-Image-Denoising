...
CSV_path = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/DnCNN_folder/lr7_weights_epochs/'
...
for d in ['/device:CPU:0','/device:GPU:0']:#, '/device:GPU:1', '/device:GPU:2', '/device:GPU:3']:
    with tf.device(d):
        path_save_model = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/DnCNN_folder/lr7_weights_epochs/Saved_model/'
        batch_size = 4
        im_height = 256 #M
        im_width = 256  #N
...
callbacks = [scheduler_callback, csv_logger,
        ModelCheckpointLight('model_dncnn_multiple_gpus_lr7_change_weights.h5', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)] #removed early stop
...
def train(epochs):
    ...
    io.savemat('Estimated_result_c1_lr7_weights_epochs.mat', dict([('Estimated_result_c1_lr7_weights_epochs',test_results)]))
    ...
    load_weight_light(model,'model_dncnn_multiple_gpus_lr7_change_weights.h5')
    ...
    tf.train.write_graph(frozen_graph, './', 'dncnn_model_lr7_v1.pbtxt', as_text=True)
    tf.train.write_graph(frozen_graph, './', 'dncnn_model_lr7_v1.pb', as_text=False)
...
if __name__ == "__main__":
    ...
    path_for_images = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/DnCNN_folder/lr7_weights_epochs/plots/'
    path_for_results = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/DnCNN_folder/lr7_weights_epochs/results/'
