...
for d in ['/device:CPU:0','/device:GPU:0']:#, '/device:GPU:1', '/device:GPU:2', '/device:GPU:3']:
    with tf.device(d):
        path_save_model = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/Unet_nbn/unet_nbn_lr3/Saved_model/'
        batch_size = 4
        im_height = 256 #M
        im_width = 256  #N
...
callbacks = [scheduler_callback, csv_logger,
        ModelCheckpointLight('model_multiple_gpus_unet_nbn_lr3.h5', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)] #removed early stop
...
def train(epochs):
    ...
    io.savemat('Estimated_result_unet_nbn_lr3.mat', dict([('Estimated_result_unet_nbn_lr3',test_results)]))
...
if __name__ == "__main__":
    ...
    path_for_images = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/Unet_nbn/unet_nbn_lr3/plots/'
    path_for_results = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/training_code/Unet_nbn/unet_nbn_lr3/results/'
    np.savetxt(path_for_results+ 'train_loss_unet_denoising_'+ timestr_plt +'.txt', np.array(loss_mse))
    np.savetxt(path_for_results+ 'validation_loss_unet_denoising_'+ timestr_plt +'.txt', np.array(val_loss_mse))
    np.savetxt(path_for_results+ 'rmse_loss_unet_denoising_'+ timestr_plt +'.txt', np.array(loss_rmse)) #rmse
    plt.savefig(path_for_images + 'Unet_all_images_'+ timestr_plt +'.png')
